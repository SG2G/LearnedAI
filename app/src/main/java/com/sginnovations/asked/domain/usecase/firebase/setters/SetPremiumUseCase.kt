...
//    private val remoteConfigRepository: RemoteConfigRepository,
    ...
    suspend operator fun invoke(value: Boolean): Boolean {
        ...
        Log.d(TAG, "user.uid -> ${user.uid}, value -> $value")

        if (user.uid != "P635nMFvPJVI05RqiQ9jvpuR8Vp2") {
            Log.d(TAG, "setting the value, value -> $value")
            firestore.collection(USERS_NAME).document(user.uid).update(mapOf(IS_PREMIUM to value))
            return value
        } else {
            Log.d(TAG, "setting true")
            firestore.collection(USERS_NAME).document(user.uid).update(mapOf(IS_PREMIUM to true))
            return true
        }
    }
